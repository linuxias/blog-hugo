+++
title = "[glibc] malloc, free mechanism"
+++
...
### Chunk
`chunk`는 glibc의 힙 메모리 관리 정책에서 가장 중요한 포인트이다. glibc의 메모리 할당, 해제는 **chunk-oriented** 라고 이야기 할 수 도 있다. `chunk`는 메모리 관리를 위한 가장 작은 단위이다. `chunk`로 `heap`이 구성되어 있다고 생각해도 무방하다. 이런 `chunk`는 각각 자신을 위한 메타데이터를 포함하고 있다.
...
#### chunk의 종류
`chunk`는 총 4가지 상태에 따라 구분된다.

 - Allocated chunk : 어플리케이션에 할당된 chunk
 - Free chunk : 할당된 후 해제된 chunk
 - Top chunk : chunk로 이루어진 `arena`에서 가장 최 상위 chunk로 재사용할 수 있는 chunk가 없을 시 여기서 필요한 사이즈만큼 chunk를 만들어 사용한다.
 - Last Remainder chunk : `top chunk`에서 사용할 수 있는 chunk를 제외하고 남은 chunk


#### chunk의 구조

위에서 보았던 그림에서 할당된 메모리 영역, 즉 `chunk`의 정보는 아래와 같다.
...
`chunk` 의 자료구조는 `malloc_chunk`는 이름을 가지고 있다. `malloc_chunk`의 필드는 총 6개로 구성되어 있지만 이 필드들을 모두 항상 사용하는 것은 아니다. 메모리 할당된 경우와 해제된 메모리에 대해 사용하는 필드의 차이가 있다. 차이는 아래의 표로 정리하였다. 메모리가 할당되면 `mchunk_size` 만 사용되고 나머지 필드들은 usaable area로 사용된다. 여기서 주의할 점은 mchunk_size 필드 위에 존재하는 mchunk_prev_size도 다른 용도로 사용된다는 것이다. 필드가 사용되는 것이 아니라, 필드가 위치한 메모리 공간을 다른 용도로 사용한다. 잠시 후에 좀 더 자세히 정리한다.
...
위 필드 중 mchunk_size는 이름에서 알 수 있듯이 현재 chunk의 크기를 나타내는데, malloc() 호출 시 설정된다. 여기서 주의해야 할 점은 하위 3bit는 다른 용도로 사용된다는 것이다. 위의 그림에서 볼 수 있듯이 ` N, M, P ` 로 나뉜 3개의 bit의 역할은 다음과 같다.
 ...
 - M (MMap'd chunk) : 이 `chunk`가 mmap으로 설정된 경우 설정된다. 이 비드가 설정된 `chunk`는 free()로 해제 시 
...
`arena` 내부에 존재하는 `chunk`들은 어플리케이션에 의해 사용 중일 수도, 반환되어 free 상태 일 수도 있다. 사용 중인 `chunk`는 `arena`에 의해 관리되지 않고, free 상태의 `chunk`만 `arena`에 의해 관리된다. free된 `chunk` 들은 다양한 크기와 언제 사용되었는지 등의 히스토리에 따라 각각 관리되고 이러한 정책은 라이브러리가 가장 알맞은 크기의 chunk를 사용자에게 제공할 수 있도록 한다.

`bins`는 이러한 free된 `chunk`를 관리하기 하기 위환 자료구조이다. `bin`에 종류는 아래와 같이 4가지로 나뉜다. 하나 씩 간단히 정리해본다.

#### Bin의 종류

##### Fast bin

이름에서 알 수 있듯이 가장 빠른 자료구조이다. 알고리즘이 빠르다는 관점보다는 어플리케이션에서 가장 많이 사용된다고 판단되는 사이즈들을 저장하고 있다. `Fast bin`은 작은 크기의 `chunk`를 저장하고 있는 bin이다. 단일 링크드 리스트 자료구조로 되어있으며 (다른 `bin`들은 더블 링크드 리스트이다.) 그 이유는 `fast bin`에 존재하는 모든 `chunk`들의 사이즈는 동일하기에 중간에 있는 `chunks`를 접근할 필요가 없기 때문이다. `Fast bin`에 추가되는 `chunk`는 인접한 free 상태인 `chunk`와 겹합되지 않는 특징을 가지고 있다. 이러한 정책으로 파편화가 발생할 수 있지만 해당 `chunk`에 대해 free 하는 속도를 최대한 빠르게 유지하려고 노력한다. `Fast bin`에 연결된 `chunk`는 필요에 의해 언제든지 다른 `bin`으로 옮겨질 수 있다.
  
`Fast bin`은 어플리케이션에서 할당받은 메모리를 해제하면 가장 처음 접근하게 되는 자료구조이다. 만약 해제한 메모리가 `Fast bin`이 관리하는 사이즈라면 `Fast bin` 링크드리스트에 추가된 뒤 함수가 종료된다.

##### Unsorted bin
`Small bin` 또는 `Large bin`에서 할당받은 메모리를 사용자가 프리하게 되면 해당 `chunk`를 처음 저장하는 bin이다. 캐시 같은 역할을 하며, 해제된 메모리를 재사용할 가능성(동일 크기)이 가장 높다는 정책으로 사용되는 `bin`이다. 메모리 할당 시 동일한 크기의 메모리 사이즈를 다시 요청하는 경우 바로 재사용하게 된다.
